...
class BaseDriver(DebugInterfaceDriver):
    def __init__(self, args: argparse.Namespace, row: dict[str, Any],
        ...
        self.trace = trace
        self.rest = R.Rest(args.rest_host, timeout=12.0)
...
class TelnetDebugDriver(BaseDriver):
    ...
    def reset_from_debug_ui(self):
        self.send_key("C=+X")
        dbg._wait_for_c64_ready(self.args.rest_host, timeout=10.0)
...
def run_preflight(args: argparse.Namespace, artifact_dir: Path) -> dict[str, Any]:
    ...
    results: dict[str, Any] = {
        "rest_liveness": R.Rest(args.rest_host).alive(),
...
def write_final_report(args: argparse.Namespace, artifact_dir: Path, ledger: Ledger,
    ...
    verdict = "PRODUCTION READY" if all_done and all_pass and opcode_pass else "NOT PRODUCTION READY"
    ...
    def count_summary(key: str) -> list[str]:
        ...
        for value in values:
            ...
            counts = {status: sum(1 for row in subset if row["status"] == status)
                      for status in ("PASS", "FAIL", "BLOCKED_WITH_EVIDENCE", "PENDING")}
            lines_out.append(
...
def run_alert_scope(args: argparse.Namespace, artifact_dir: Path) -> int:
    ...
    try:
        client = R.Rest(args.rest_host, timeout=3.0)
...
def main() -> int:
    ...
    for row in ledger.rows:
        ...
        print(progress_line(ledger.rows, opcode_status), flush=True)
        if row["status"] != "PASS" and stop_after_cell_failure(args):
    ...
    if args.strict:
        ...
        rows_pass = all(row["status"] == "PASS" for row in ledger.rows)
        if not rows_done or not rows_pass or opcode_status != "PASS":
            return 1
